// ─── Collectible Image Signed-URL Route ───────────────────────────────────────
// GET /api/collectible/image?path=hunt-1/art.png
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (error || !data?.signedUrl) {
      console.error('[collectible/image] signed URL error:', error?.message ?? 'no signedUrl returned')
      return NextResponse.json(
    ...
    console.log('[collectible/image] signed URL generated for path:', path)
    return NextResponse.json({ signedUrl: data.signedUrl as string })

  } catch (err) {
    console.error('[collectible/image] unexpected error:', err)
    return NextResponse.json(
