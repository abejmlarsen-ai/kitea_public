...
export async function POST(req: NextRequest) {
  try {
    ...
    if (existing) {
      console.log('[collectible/mint] already minted — edition_number:', existing.edition_number)
    ...
    console.log('[collectible/mint] inserting minted row — edition_number:', edition_number)
    ...
    if (insertError) {
      console.error('[collectible/mint] insert error:', insertError.message)
      return NextResponse.json({ error: 'Failed to save collectible' }, { status: 500 })
    }

    console.log('[collectible/mint] minted row inserted — id:', newRow.id, '| edition_number:', newRow.edition_number)
